...
export default function DisplayColorElement({isDay, color, skylineUrl}: DisplayColorElementProps) {
    ...
    if (dayFlag) {
    return <h3>The sky is pretty close to hex color <ToolTip skyColor={color} skylineUrl={skylineUrl}><span className="underline" style={{color: color}}>{color}</span></ToolTip>.</h3>
    }
